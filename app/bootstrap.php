...
// No real mail transport in CI/test environments (no configured MTA) - swap
// in a no-op mailer there instead of letting every notification email throw
// and derail whatever request triggered it. Never set in production.
if (getenv('MOCK_MAILER')) {
	$configurator->addConfig(__DIR__ . '/config/config.mock-mailer.neon');
}
