...
class RatingControl extends Control
{
	...
	{
		$key = array_find_key($this->colorIntervals, fn ($interval) => $ratio <= $interval);
		return $key !== null ? $this->colorClasses [ $key ] : "rating-1";
	}

	#[\Deprecated('unreachable and broken (returns an undefined $html) - not called from anywhere')]
